...
void setup_wave(celerity::queue &queue, celerity::buffer<float, 2> u, sycl::float2 center, float amplitude, sycl::float2 sigma) {
	queue.submit([&](celerity::handler &cgh) {
		celerity::accessor dw_u{u, cgh, celerity::access::one_to_one{}, celerity::read_write, celerity::no_init};
...
void step(celerity::queue &queue, celerity::buffer<T, 2> up, celerity::buffer<T, 2> u, float dt, sycl::float2 delta) {
	queue.submit([&](celerity::handler &cgh) {
		...
		cgh.parallel_for<KernelName>(size, [=](celerity::item<2> item) {
			...
			const float lap = (dt / delta.y()) * (dt / delta.y()) *
								  ((r_u[{py, item[1]}] -
									r_u[item]) -
								   (r_u[item] -
									r_u[{my, item[1]}])) +
...
void stream_open(celerity::queue &queue, size_t X, size_t Y, size_t num_samples, celerity::experimental::host_object<std::ofstream> os) {
	queue.submit([&](celerity::handler &cgh) {
...
void stream_append(celerity::queue &queue, celerity::buffer<T, 2> up, celerity::experimental::host_object<std::ofstream> os) {
	...
	queue.submit([&](celerity::handler &cgh) {
		...
		celerity::experimental::side_effect os_eff{os, cgh};
		cgh.host_task(celerity::on_master_node, [=] { 
			printf("Writing %zu items of size %zu.\n", range.size(), sizeof(T));
			os_eff->write(reinterpret_cast<const char*>(up_r.get_pointer()), range.size() * sizeof(T)); });
	});
}
...
int main(int argc, char *argv[]) {
	...
	const wave_sim_config cfg = ([&]() {
		...
		for (auto it = args.cbegin(); it != args.cend(); ++it) {
			if (get_cli_arg(args, it, "-X", result.X, atol) || get_cli_arg(args, it, "-Y", result.Y, atol) ||
				get_cli_arg(args, it, "-T", result.T, atoi) || get_cli_arg(args, it, "--dt", result.dt, atof) ||
	...
	queue.submit([&](celerity::handler &cgh) {
		cgh.host_task(celerity::on_master_node, [=] {
			std::cout << "X: " << cfg.X << ", Y: " << cfg.Y << std::endl;
		});
	});

	// celerity::detail::mpi_communicator comm(celerity::detail::collective_clone_from, MPI_COMM_WORLD);
	// celerity::range<1> node_range {comm.get_num_nodes()};
	int node_count;
	MPI_Comm_size(MPI_COMM_WORLD, &node_count);
	celerity::range<1> node_range{static_cast<size_t>(node_count)};
	// std::cout << "Num. nodes: " << comm.get_num_nodes() << std::endl;
	...
	setup_wave(queue, u, {cfg.Y / 4.f, cfg.X / 4.f}, 1, {cfg.Y / 8.f, cfg.X / 8.f});
	zero(queue, up);
	...
	if (cfg.output_sample_rate > 0) {
		stream_open(queue, cfg.X, cfg.Y, num_samples, os);
	...
	while (t < cfg.T) {
		...
		if (cfg.output_sample_rate > 0) {
			if (++i % cfg.output_sample_rate == 0) {
				stream_append(queue, up, os);
			}
		}
		std::swap(u, up);
		t += cfg.dt;
	...
	float elapsed_init = std::chrono::duration_cast<std::chrono::microseconds>(init_end - init_begin).count() / 1000000.0;
	float elapsed_loop = std::chrono::duration_cast<std::chrono::microseconds>(loop_end - loop_begin).count() / 1000000.0;

	// Reduce timers
	celerity::buffer<float, 0> max_loop_buf;
	celerity::buffer<float, 0> max_init_buf;
	queue.submit([&](celerity::handler &cgh) {
		// celerity::accessor elapsed_loop_rw{elapsed_loop_buf, cgh, celerity::read_write_host_task};
		const auto max_init_r = celerity::reduction(max_init_buf, cgh, sycl::maximum<float>(), celerity::property::reduction::initialize_to_identity{});
		const auto max_loop_r = celerity::reduction(max_loop_buf, cgh, sycl::maximum<float>(), celerity::property::reduction::initialize_to_identity{});
		cgh.parallel_for<class linearize_and_accumulate>(node_range, max_init_r, max_loop_r, [=](celerity::item<1> item, auto &max_init, auto &max_loop) {
			// max.combine(*elapsed_loop_rw);
			max_init.combine(elapsed_init);
			max_loop.combine(elapsed_loop);
		});
	});

	queue.submit([&](celerity::handler &cgh) {
		celerity::accessor max_init{max_init_buf, cgh, celerity::read_only_host_task};
		celerity::accessor max_loop{max_loop_buf, cgh, celerity::read_only_host_task};
		cgh.host_task(celerity::on_master_node, [=] {
			std::cout << "Max Init time = " << *max_init << "s" << std::endl;
			std::cout << "Max Loop time = " << *max_loop << "s" << std::endl;
		});
	});

	// Close output stream
	if (cfg.output_sample_rate > 0) {
		stream_close(queue, os);
	}
