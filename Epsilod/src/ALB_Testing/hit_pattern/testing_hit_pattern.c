	...
	__ctrl_block__(device_selection_file) {
		...
		if (hit_layImActive(lay)) {
			...
			*/
			...
			for (int i = 0; i < tileData.tileMat.acumCard; i++)
				hit(tileData.tileMat, i) = 125;

			float sameWeightsArray[hit_NProcs];
			double sumaSame = 0;
			for (int k=0; k<hit_NProcs; k++) {
				sumaSame += 1;
			} 
			for (int k=0; k<hit_NProcs; k++) {
				sameWeightsArray[k] = (float) (sumaSame / 1);
			}
			HitWeights sameWeights = hitWeights( hit_NProcs, sameWeightsArray );
			
			HitLayout newLay;
			newLay = hit_layout_freeTopo(plug_layDimWeighted_Blocks, hit_topology(plug_topPlain), lay.origShape, 0, sameWeights);
			HitShape newShape;
			newShape = hit_layShape(newLay);

			/* CREATE NEW TILES */
			TileData newTiles;
			TileCommon newCommon;
			newCommon.borderInActive = malloc(common.numBorders * sizeof(int));

			/* COMPUTE NEW TILE */
			allocateTileData(&newTiles, newCommon, dims);
			computeSingleTileData(&newTiles, &newCommon, comm, shp, newShape, newLay, stencil, dims, stencilData, borderLow, borderHigh, &globalMat);
			computeCommonData(&newCommon, shp, newShape, newLay, stencil, dims, stencilData, borderLow, borderHigh);


			if (hit_Rank == 0) {
				printf("\nPartition weights = {");
				for (int i = 0; i < sameWeights.num_procs; i++)
					printf(" %f,", sameWeights.ratios[i]);
				printf("\b }\n");
				fflush(stdout);
			}
			...
			for (int i = 0; i < newTiles.tileMat.acumCard; i++){
				if(!(hit(newTiles.tileMat, i) == 125)){
					returnValue = 21;
				}
			}


			float diffWeightsArray[hit_NProcs];
			double sumaDiff = 0;
			for (int k=0; k<hit_NProcs; k++) {
				sumaDiff += (k+1);
			} 
			for (int k=0; k<hit_NProcs; k++) {
				diffWeightsArray[k] = (float) (sumaDiff / (k+1));
			}
			HitWeights diffWeights = hitWeights( hit_NProcs, diffWeightsArray );


			newLay = hit_layout_freeTopo(plug_layDimWeighted_Blocks, hit_topology(plug_topPlain), lay.origShape, 0, diffWeights);
			newShape = hit_layShape(newLay);
			...
			if (hit_Rank == 0) {
				printf("\nPartition weights = {");
				for (int i = 0; i < diffWeights.num_procs; i++)
					printf(" %f,", diffWeights.ratios[i]);
				printf("\b }\n");
				fflush(stdout);
			}
			...
			Ctrl_MoveTo(comm, tileData.tileMat);
			Ctrl_WaitTile(comm, tileData.tileMat);
			Ctrl_MoveTo(comm, tileData.tileInnerLocal);
			Ctrl_WaitTile(comm, tileData.tileInnerLocal);
