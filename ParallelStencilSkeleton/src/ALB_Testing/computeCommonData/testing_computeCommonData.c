	...
	__ctrl_block__(device_selection_file) {
		...
		if (hit_layImActive(lay)) {
			...
			*/
			...
			//ADD TESTING HERE
			TileCommon data;
			data.borderInActive = malloc(common.numBorders * sizeof(int));
			computeCommonData(&data, shp, shpLayout, lay, stencil, dims, stencilData, borderLow, borderHigh);
			
			if(
				!(data.num_tiles_total == (1 + 1 + 2 * common.numBorders + dims * 2)) ||
				!(data.numBorders == ((int)pow(3, dims))) ||
				!(data.dims == dims) 
			){
				returnValue = 21;
			}

			for(int i = 0; i<common.numBorders; i++){
				if(!(common.borderInActive[i] == data.borderInActive[i])){
					returnValue = 22;
				}
			}
