	...
	__ctrl_block__(device_selection_file) {
		...
		if (hit_layImActive(lay)) {
			...
			*/
			...
			//ADD TESTING HERE
			HitShape expShape;
			HitShape expShapeHalos;
			expShape = expandShapeBorders(&globalMat, borderLow, borderHigh, shpLayout);
			expShapeHalos = expandShapeBordersAndHalos(&globalMat, borderLow, borderHigh, shpLayout);

			for(int i = 0; i < dims; i++){
				if(
					!(expShape.info.sig.sig[i].begin == tileData.io_tile.shape.info.sig.sig[i].begin) ||
					!(expShape.info.sig.sig[i].end == tileData.io_tile.shape.info.sig.sig[i].end) ||
					!((expShape.info.sig.sig[i].end - expShape.info.sig.sig[i].begin + 1) == (tileData.io_tile.shape.info.sig.sig[i].end - tileData.io_tile.shape.info.sig.sig[i].begin + 1))
				){
					returnValue = 21;
				}

				if(
					!(expShapeHalos.info.sig.sig[i].begin ==shpExpanded.info.sig.sig[i].begin) ||
					!(expShapeHalos.info.sig.sig[i].end == shpExpanded.info.sig.sig[i].end) ||
					!((expShapeHalos.info.sig.sig[i].end - expShapeHalos.info.sig.sig[i].begin + 1) == (shpExpanded.info.sig.sig[i].end - shpExpanded.info.sig.sig[i].begin + 1)) 
				){
					returnValue = 22;
				}
			}
			...
			HitShape nullShape = HIT_SHAPE_NULL_STATIC;
			expShape = expandShapeBorders(&globalMat, borderLow, borderHigh, nullShape);
			expShapeHalos = expandShapeBordersAndHalos(&globalMat, borderLow, borderHigh, nullShape);

			for(int i = 0; i < dims; i++){
				if(
					!(expShape.info.sig.sig[i].begin == -1) ||
					!(expShape.info.sig.sig[i].end == -2) ||
					!((expShape.info.sig.sig[i].end - expShape.info.sig.sig[i].begin + 1) == 0)
				){
					returnValue = 25;
				}

				if(
					!(expShapeHalos.info.sig.sig[i].begin == -1) ||
					!(expShapeHalos.info.sig.sig[i].end == -2) ||
					!((expShapeHalos.info.sig.sig[i].end - expShapeHalos.info.sig.sig[i].begin + 1) == 0) 
				){
					returnValue = 26;
				}
			}
