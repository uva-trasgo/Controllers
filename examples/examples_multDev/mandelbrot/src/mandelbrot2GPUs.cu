...
#pragma omp single nowait
{
	...
	if ( argc != 10 ) {
        ...
        float perCPU = atoi( argv[8] );
        float perGPU = atoi( argv[9] );
	...
	double time = wtime();
	// Compute
        CAL_MCntrlLaunch(cntrlMult, threads,0, Mandelbrot, 8, &hit_Ctile(resultx,0), xsize,ysize, begin_x, begin_y, end_x, end_y, 0);
