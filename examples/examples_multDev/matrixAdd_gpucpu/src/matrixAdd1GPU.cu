...
#pragma omp single nowait
{
	...
	hit_CtileDomainAlloc(A, 1, float, domains_cntrl);	
	hit_CtileDomainAlloc(B, 1, float, domains_cntrl);	
	hit_CtileDomainAlloc(C, 1, float, domains_cntrl);	
	...
	int dim_1= hit_shapeSigCard(domains_cntrl[1],1);
        ...
        for (int i=0; i<dim_0; i++){
                for (int j=0; j<dim_1; j++) {
                     hit_tileElemAt(A.DS_dev[1], 2, i, j) = 2.12;
	...
	int dim_1_CPU= hit_shapeSigCard(domains_cntrl[0],1);
        for (int i=0; i<dim_0_CPU; i++){
                for (int j=0; j<dim_1_CPU; j++) {
                     hit_tileElemAt(A.DS_dev[0], 2, i, j) = 2.12;
...
/*
        for (int i=0; i<dim_0; i++)
                for (int j=0; j<dim_1; j++) 
                        if ( fabs( hit_tileElemAt((C.DS_dev[1]), 2, i, j) - 2.32 * NITER ) > 0.01 )
                                fprintf(stderr, "ERROR: Resultado C[%d,%d] = %f\n", i, j, 
						hit_tileElemAt((C.DS_dev[1]), 2, i, j) );
*/		
        for (int i=0; i<dim_0_CPU; i++)
                for (int j=0; j<dim_1_CPU; j++) 
                        if ( fabs( hit_tileElemAt((C.DS_dev[0]), 2, i, j) - 2.32 * NITER ) > 0.01 )
