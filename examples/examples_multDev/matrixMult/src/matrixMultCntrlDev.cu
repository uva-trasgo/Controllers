...
/* CPU KERNELS */
CAL_KERNEL_CPU(MatMult_CPU, 3, IN, HitTile_float*, A, IN, HitTile_float*, B, OUT, HitTile_float*, C){
        int k=0;
        for(k=0; k< hit_tileDimCard(*C,1); k++ ){

                hit_tileElemAt(*C, 2, threadId.x, threadId.y) =hit_tileElemAt(*C, 2, threadId.x, threadId.y) +
                         hit_tileElemAt(*A, 2, threadId.x, k) * hit_tileElemAt(*B, 2, k, threadId.y);
        }
...
#pragma omp single nowait
{
	...
	printf("\n%f perCPU %f perGPU  Cards: %d %d\n", perCPU,perGPU,hit_shapeSigCard(domains_cntrl[0],0), hit_shapeSigCard(domains_cntrl[1],0)); 
#endif
	...
	*/
	...
	// Compute
	CAL_MCntrlLaunch(cntrlMult, threads,0, MatMult_CPU, 3, &hit_Ctile(A,0), &hit_Ctile(B,0), &hit_Ctile(C,0));
