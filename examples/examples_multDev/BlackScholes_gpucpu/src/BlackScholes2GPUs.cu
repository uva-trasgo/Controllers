...
int main(int argc, char *argv[]) {
	...
	printf("\n Cards: %d %d\n", hit_shapeSigCard(domains_cntrl[0],0), hit_shapeSigCard(domains_cntrl[1],0)); 
#endif
	...
	for (i = 0; i < NUM_ITERATIONS; i++){
		...
		CAL_MCntrlLaunch(cntrlMult, threads,0, BlackScholesKernel_GPU, 7, &hit_Ctile(h_CallResult,0), &hit_Ctile(h_PutResult,0), &hit_Ctile(h_StockPrice,0), &hit_Ctile(h_OptionStrike,0), &hit_Ctile(h_OptionYears,0), RISKFREE, VOLATILITY );
