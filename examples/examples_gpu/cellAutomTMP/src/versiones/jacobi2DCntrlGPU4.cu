        ...
        #pragma omp single 
         {
			...
			if ( argc != 6 ) {
				fprintf(stderr, "\nUsage: %s <numRows> <numColumns> <numIterations> <GPU> <Version>\n", argv[0]);
				exit(EXIT_FAILURE);
			}		
...
TimerStop(tComm);					
					...
					if (loopIndex>0) TimerStop(tComp);					
TimerContinue(tComp);	
...
TimerStop(tTotal);	//ttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt
				...
				printf("----> Rows, Cols, iter: %d,%d,%d\n\n", rows, columns, numIter );
				printf("----> Total Clock %.8lf\n", TimerGetTime(tTotal));
				printf("----> Comp Clock %.8lf\n", TimerGetTime(tComp));
				printf("----> Comm Clock %.8lf\n", TimerGetTime(tComm));
				TimerDestroy(tTotal);
