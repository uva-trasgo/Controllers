...
/*Relojes*/
HitClock	mainClockMPI;
HitClock	productClockMPI;
//double  mainClock;
//double  initClock;
...
int main(int argc, char *argv[]) {
	...
	NUM_ITERATIONS = (int) (NUM_ITERATIONS/size); 
	GPU = rank;
	printf("rank %d;   size %d;  iter %d\n", rank, size, NUM_ITERATIONS);
		...
		HitLayout matLayout = hit_layout( plug_layBlocks, topo, hit_tileShape( h_OptionYears ) );
		...
		hit_clockStop( mainClockMPI );		
		hit_clockReduce( matLayout, mainClockMPI );    
		hit_clockPrintMax( mainClockMPI );
		...
		//hit_clockReduce( matLayout, tTotal );		
