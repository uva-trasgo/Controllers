/**
 * @file Sobel_YUV_Hip_Ctrl_MTF.cpp
 * @brief SobelYUV: Ctrl HIP mem to file version
 *
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < Num_Frames; i++) {
		...
		for (int j = 0; j < N_IMG; j++) {
			hipHostMalloc((void **)(&(buffer_read[i][j])), sizes[j] * sizeof(BYTE));
		}
	}
