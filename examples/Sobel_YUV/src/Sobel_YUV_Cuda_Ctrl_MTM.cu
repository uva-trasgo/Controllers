/**
 * @file Sobel_YUV_Cuda_Ctrl_MTM.cu
 * @brief SobelYUV: Ctrl CUDA mem to mem version
 *
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < Num_Frames; i++) {
		...
		for (int j = 0; j < N_IMG; j++) {
			cudaMallocHost((void **)(&(buffer_read[i][j])), sizes[j] * sizeof(BYTE));
			cudaMallocHost((void **)(&(buffer_write[i][j])), sizes[j] * sizeof(BYTE));
		}
	}
	...
	for (int i = 0; i < Num_Frames; i++) {
		for (int j = 0; j < N_IMG; j++) {
			cudaFreeHost(buffer_read[i][j]);
			cudaFreeHost(buffer_write[i][j]);
		}
		free(buffer_read[i]);
		free(buffer_write[i]);
	}
